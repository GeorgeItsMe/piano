...
function renderNotes(notes, kind = '', animate = true){
  ...
  for (const n of list){
    // у каждой ноты может быть своё состояние: заданная — чернилами,
    // ошибочная — красным, и обе стоят на стане одновременно
    const k = n.kind || kind;
    const ghost = k === 'ghost';
    const staff = n.staff || (L.ladder ? 'treble' : staffOf(n.midi));
    const y = yOf(n.dia, staff);
    // сдвигаем вбок и соседнюю ступень, и совпавшую — иначе головки сольются
    const off = (prevD !== null && n.dia - prevD <= 1 && prevOff === 0) ? 1 : 0;
    prevD = n.dia; prevOff = off;
    const cx = L.noteX + off * 1.55 * GAP;

    for (const l of ledgersFor(n.dia, staff)){
      const key = `${staff}:${l}`, prev = ledgers.get(key);
      if (prev){
        // линейку делят обе ноты — красить её в цвет одной нечестно
        if (prev.k !== k) prev.k = '';
        prev.a = Math.min(prev.a, off);
        prev.b = Math.max(prev.b, off);
      } else ledgers.set(key, { y: yOf(l, staff), k, a: off, b: off });
    }
    ...
    if (n.alter !== 0 || ghost){
      ...
      if (a){
        ...
        out += glyphPath(a.g, ghost ? 'ghost' : 'glyph' + (k ? ' glyph--' + k : ''),
    ...
    const head = ghost ? 'ghost' : 'head' + (k ? ' head--' + k : '');
  ...
  for (const [, l] of ledgers){
    const cls = l.k === 'ghost' ? 'ghost-ledger' : 'ledger' + (l.k ? ' ledger--' + l.k : '');
    led += `<line class="${cls}" x1="${r(L.noteX - lw + l.a * 1.55 * GAP)}" y1="${r(l.y)}" ` +
           `x2="${r(L.noteX + lw + l.b * 1.55 * GAP)}" y2="${r(l.y)}"/>`;
  }
...
function drawLink(midis){
  ...
  let out = '';
  for (const item of midis){
    const midi = typeof item === 'number' ? item : item.midi;
    const cls = typeof item === 'object' && item.kind ? ' class="' + item.kind + '"' : '';
    ...
    out += `<path${cls} d="M${r(x1)} ${r(y1)} C ${r(x1)} ${r(y1 + dy)}, ${r(x2)} ${r(y2 - dy)}, ${r(x2)} ${r(y2)}"/>` +
           `<circle${cls} cx="${r(x2)}" cy="${r(y2)}" r="2.6"/>`;
  }
...
function setMode(mode){
  T.mode = mode; T.target = null; T.wrong = 0; T.placeAlter = 0; T.miss = null;
...
function nextRound(){
  T.wrong = 0; T.lock = false; T.placeAlter = 0; T.miss = null;
  clearMarks(); clearNotes(); answerEl.innerHTML = '';
  // после далёкого промаха стан мог раздвинуться — возвращаем обычный размер
  const b = baseSpan();
  if (!SPAN || SPAN.dMin !== b.dMin || SPAN.dMax !== b.dMax) renderStaff();
...
/* Промах не стираем, а показываем: заданная нота остаётся чернилами, а рядом
   встаёт красная — та, куда ученик попал. Видно не «мимо», а насколько мимо. */
function showMiss(){
  const notes = [T.target];
  if (T.miss) notes.push(Object.assign({}, T.miss, { kind:'bad' }));
  renderNotes(notes, '', false);
  if (T.miss) requestAnimationFrame(() => drawLink([{ midi:T.miss.midi, kind:'bad' }]));
}
...
function pressKey(midi, fromHold){
  ...
  if (T.mode === 'noteToKey'){
    ...
    if (ok){
      ...
      T.miss = sp;
      showMiss();
      cue(`<b>Мимо.</b> Красная — ${fullName(sp)}, её ты и нажал`, 'bad');
...
function answerName(li, btn){
  ...
  if (ok){
    ...
    // ставим на стан ту ноту, которую он назвал — в ближайшей к заданию октаве
    const dia = li + 7 * Math.round((T.target.dia - li) / 7);
    const alter = T.needAlter ? T.placeAlter : 0;
    T.miss = { midi: midiFromDia(dia, alter), dia, alter, li, oct: Math.floor(dia / 7) };
    showMiss();
    cue(`<b>Нет.</b> Красная — ${noteName(T.miss)}, как ты назвал. Чёрная стоит на другой ступени`, 'bad');
...
const relink = () => {
  ...
  raf = requestAnimationFrame(() => {
    ...
    else if (T.miss) drawLink([{ midi:T.miss.midi, kind:'bad' }]);
